...
TestObject.overrideWD = function (wd, appiumZipUrl) {
  ...
  wd.promiseChainRemote = async function (host, port, ...args) {
    const driver = await originalPromiseChainRemote(TestObject.HOST, TestObject.PORT, ...args);
...
TestObject.usingTestObject = function (wd, appiumDir = process.env.PWD) {
  ...
  after(async function () { // jshint ignore:line
    log.info(`Deleting ${process.env.PWD} from S3`);
    await zip.deleteZip(appiumS3Object.Key);
    log.info(`Deleted ${process.env.PWD} from S3`);
